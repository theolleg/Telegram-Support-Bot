...
def fwd_handler(user_id, bot, message, user_info):
    ...
    if message.content_type == 'text':
        msg = bot.send_message(config.support_chat, "[{0}{1}](tg://user?id={2}) (#id{2}) | {3}\n{4} {5} {7}\n\n{6}".format(
            ...
            message.from_user.id, lang_emoji, user_info['organization'], user_info['card_number'], message.text, user_info["ticker_number"] if user_info["ticker_number"] else ''), parse_mode='Markdown', disable_web_page_preview=True)
