...
def register_telephone(message: telebot.types.Message):
    mysql.update_telephone(message.from_user.id, message.text.replace('+', '').replace('(', '').replace(')', '').replace('-', '').replace(' ', ''))
    ...
    if message.chat.id == config.support_chat:
        ...
        for user in mysql.open_tickets:
            ...
            if manager_id is not None:
                ...
                managerName = '[{0}](tg://user?id={1})'.format(manager.first_name, manager.id)
    ...
    try:
        ...
        if message.chat.id == config.support_chat:
            ...
            if message.reply_to_message:
                ...
                bot.reply_to(message, f'✅ Цей менеджер був заблокований на {hours} годин!')
            else:
